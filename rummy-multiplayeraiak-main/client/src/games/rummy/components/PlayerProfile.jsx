...
export const PlayerProfile = ({
  ...
  return (
    <div
      className={`relative pointer-events-auto flex flex-col items-center gap-2 p-3 rounded-lg transition-all ${isActive
    ...
    >
      ...
      <div
        className={`w-12 h-12 rounded-full flex items-center justify-center overflow-hidden ${isActive
      ...
      >
        {profilePic ? (
          ...
          <User
            className={`w-6 h-6 ${isActive ? 'text-white' : isCurrentUser ? 'text-blue-200' : 'text-slate-300'
              }`}
          />
        )}
      </div>
      ...
      <div className="text-center">
        <p
          className={`text-xs font-bold ${isActive ? 'text-amber-100' : isCurrentUser ? 'text-blue-100' : 'text-slate-200'
            } truncate`}
          style={{ maxWidth: 100 }}
        >
          {name}
        ...
        <p
          className={`text-[10px] ${isActive ? 'text-amber-200' : isCurrentUser ? 'text-blue-300' : 'text-slate-400'
            }`}
        >
          {position}
        </p>
