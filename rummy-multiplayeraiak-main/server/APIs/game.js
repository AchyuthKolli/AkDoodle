...
router.get("/tables/info", requireAuth, async (req, res) => {
  try {
    ...
    const players = await db.fetch(
      `SELECT p.user_id, p.display_name, p.seat, p.is_spectator, p.spectator_allowed, p.total_points, p.disqualified, p.eliminated_at, rp.avatar_url as profile_image_url 
